...
parameters {
  ...
  // all cauchy variables are decomposed into chi-square and inverse-gamma portions
  ...
  vector<lower = 0.0>[n_groups_stan+1] gamma_base_sq;//
  vector<lower = 0.0>[n_groups_stan+1] gamma_scale_sq;//
}
transformed parameters {
  ...
  vector<lower = 0.0>[n_groups_stan+1] gamma_sq;//gamma^2
  ...
  for(i in 1:(n_groups_stan+1)) {
    theta[i] = 1.0 / sqrt(1.0 + (1.0 / (alpha_scale_stan_sq * tau_glob_sq * lambda_sq[i] * gamma_sq[i])));
  }
